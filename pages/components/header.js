...
export function Header() {
  ...
  return (
    <div
      className={`flex justify-center content-center dark:bg-[${com_styles[0].dark}] bg-[${com_styles[0].light}] lg:px-[80px] lg:py-4 w-[100%] p-4`}
    >
      <div className="flex justify-between lg:px-8 items-center w-[100%] max-w-[1280px]">
        <span
          className={`text-3xl text-bold dark:text-[${com_styles[1].dark}] text-[${com_styles[1].light}]`}
        >
          {logo}
        </span>
        <div className=" hidden gap-6 items-center lg:flex">
          {nav_bar.map((bar) => {
            for (let i = 0; i < nav_bar.length; i++) {
              return <Link href={bar.link}>{bar.name}</Link>;
            }
          })}
          <div
            className={`w-[1px] dark:bg-[${com_styles[4].dark}] bg-[${com_styles[4].light}] h-6`}
          ></div>
          <div onClick={modeHandler} id="page_mode">
      ...
      <div className="lg:hidden">
        ...
        <div
          className={`top-0 right-0 bottom-0 transition-all duration-1000 fixed dark:bg-[${
            com_styles[0].dark
          }] bg-[${com_styles[0].light}] z-10 ease-in-out ${
            open ? "left-0" : "left-full"
          }`}
        >
          <div className="flex justify-between p-4 items-center">
            <span
              className={`text-2xl text-bold dark:text-[${com_styles[2].dark}] text-[${com_styles[2].light}]`}
            >
              {logo}
            </span>
          ...
          <div
            className={`border-y-[1px] dark:border-[${com_styles[4].dark}] border-[${com_styles[4].light}] flex flex-col gap-4 p-4`}
          >
            {nav_bar.map((bar) => (
